// * 创建类 Palette
function Palette(canvas, ctx) {
    // 创建类的对象,初始化画布内部元素默认样式
    ...
    // 创建类的对象
    Palette.prototype.init = function () { // 绘画初始化
        this.ctx.strokeStyle = this.strokeColor;
        // strokeStyle 属性设置或返回用于笔触的颜色、渐变或模式。
        this.ctx.fillStyle = this.fillColor;
        // fillStyle 属性设置或返回用于填充绘画的颜色、渐变或模式。
    };
    Palette.prototype.line = function (x1, y1, x2, y2) { // 绘制直线
        this.ctx.beginPath(); // beginPath() 方法开始一条路径，或重置当前的路径
        this.ctx.moveTo(x1 - 0.5, y1 - 0.5); // moveTo() 方法可把窗口的左上角移动到一个指定的坐标。
        this.ctx.lineTo(x2 - 0.5, y2 - 0.5); // lineTo() 方法添加一个新点，然后创建从该点到画布中最后指定点的线条（该方法并不会创建线条）。
        this.ctx.closePath(); // closePath() 方法创建从当前点到开始点的路径。
        this.ctx.stroke(); // stroke() 方法会实际地绘制出通过 moveTo() 和 lineTo() 方法定义的路径。默认颜色是黑色。
    };
    Palette.prototype.rect = function (x1, y1, x2, y2) { // 绘制矩形
        this.ctx.beginPath();
        this.ctx.rect(x1 - 0.5, y1 - 0.5, x2 - x1, y2 - y1);
        this.ctx.closePath();
        this.ctx[this.type]();
    };
    Palette.prototype.dytriangle = function (x1, y1, x2, y2) { // 绘制等腰三角形
        this.ctx.beginPath();
    ...
    Palette.prototype.zjtriangle = function (x1, y1, x2, y2) { // 绘制直角三角形
        this.ctx.beginPath();
        this.ctx.lineTo(x1, y1);
        this.ctx.lineTo(x2, y2);
    ...
    Palette.prototype.polygon = function (x1, y1, x2, y2) { // 绘制多边形
        this.ctx.beginPath();
    ...
    Palette.prototype.circle = function (x1, y1, x2, y2) { // 绘制圆形
        this.ctx.beginPath();
        var r = Math.sqrt(Math.pow(x2 - x1, 2), Math.pow(y2 - y1), 2);
    ...
    Palette.prototype.tablet = function (x1, y1, x2, y2) { // 绘制写字板
        this.ctx.lineTo(x2, y2);
        this.ctx[this.type]();
    };
    Palette.prototype.ellipse = function (x1, y1, x2, y2) { // 绘制椭圆
        this.ctx.beginPath();
    ...
    Palette.prototype.save = function (x1, y1, x2, y2) { // 保存
        location.href = canvas.toDataURL().replace('image/png', 'image/stream');
    };
    Palette.prototype.eraser = function (x1, y1, x2, y2) { // 橡皮
        this.ctx.clearRect(x2, y2, 10, 10);
    };
    Palette.prototype.revo = function (x1, y1, x2, y2) { // 撤回
    ...
    Palette.prototype.drawing = function () { // 书写绘画函数
        var that = this;
        // console.log(this);
        this.canvas.onmousedown = function (e) { // 鼠标移动画布的函数
            // * 获取鼠标起始位置
            var sx = e.offsetX; // 获取鼠标到时间源的宽度
            // console.log(sx);
            var sy = e.offsetY; // 获取鼠标到时间源的高度
            // console.log(sy);
            that.init(); // 初始化样式
            if (that.style == "tablet") {
                that.ctx.beginPath(); // beginPath() 方法开始一条路径，或重置当前的路径
                that.ctx.moveTo(sx, sy); // moveTo() 方法可把窗口的左上角移动到一个指定的坐标。
            }
            // 获取鼠标移动时的坐标
...
$(function () {
    ...
    // console.log(ctx);
    var num = 1;
    // 实例化一个Palette,引进参数canvas和ctx
    var p = new Palette(canvas, ctx);
